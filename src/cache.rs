...
use crate::drive::model::PikpakFile;

#[derive(Clone)]
pub struct Cache {
    inner: MokaCache<String, Vec<PikpakFile>>,
}
...
impl Cache {
    ...
    pub fn get(&self, key: &String) -> Option<Vec<PikpakFile>> {
        trace!(key = %key, "cache: get");
        self.inner.get(key)
    }

    pub async fn insert(&self, key: String, value: Vec<PikpakFile>) {
